...
export default function AdminPublicMaterials() {
  ...
  const tabs = [
    { key: 'upload',  label: 'Upload New Material', icon: 'upload_file' },
    { key: 'content', label: 'Published Content',   icon: 'library_books' },
  ]
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{
          ...
          {tabs.map(t => (
            <button
              key={t.key}
              onClick={() => { setActiveTab(t.key); if (t.key === 'content') setPage(1) }}
              style={{
                flex: 1,
                ...
                background: activeTab === t.key ? 'var(--brand-primary)' : 'transparent',
                color: activeTab === t.key ? '#fff' : 'var(--text-muted)',
                transition: 'all 0.2s ease',
                boxShadow: activeTab === t.key ? '0 2px 8px rgba(240,130,50,0.3)' : 'none'
              }}
            >
