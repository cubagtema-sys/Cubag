          ...
          ..._schedules.map((s) => Card(
            ...
            child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(s['vessel']?.toString() ?? s['container']?.toString() ?? 'Unnamed Cargo', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                Text('${s['port'] ?? 'Unknown Port'} · ${s['date'] ?? 'No Date'}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
              ])),
              Container(
